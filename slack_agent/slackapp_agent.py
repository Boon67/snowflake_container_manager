...
warnings.filterwarnings(
    "ignore", message="The top-level text argument is missing", category=UserWarning
)
...
ANALYST_ENDPOINT = (
    f"https://{SF_ACCOUNT}.snowflakecomputing.com/api/v2/cortex/analyst/message"
)
...
def search_chunks(question: str, customer: str, k: int = 5) -> List[Dict[str, Any]]:
    hits = search_svc.search(
        query=question, columns=["CHUNK", "CUSTOMER_NAME", "RELATIVE_PATH"], limit=k
    ).results
...
def run_agent(question: str, customer: str) -> Dict[str, Any]:
    def run_agent(question: str, customer: str) -> Dict[str, Any]:
        """
        Processes a user question in the context of a specific customer and returns an answer, SQL statement (if applicable), and a summary.

        The function determines whether the question is SQL-related (e.g., asking for counts, lists, averages, or to "show" data) or requires retrieval-augmented generation (RAG) using contextual search. For SQL-related questions, it generates a SQL query, executes it, and summarizes the results. For other questions, it retrieves relevant context chunks, sends them along with the question to an analyst endpoint, and returns the generated answer.

        Args:
            question (str): The user's question to be answered.
            customer (str): The customer identifier used for context retrieval.

        Returns:
            Dict[str, Any]: A dictionary containing:
                - "answer": The answer as a string (table or free text).
                - "sql": The SQL statement used to generate the answer, or None if not applicable.
                - "summary": A summary of the answer, or None if not available.
        """
    ...
    if re.search(
            ...
            "messages": [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "You are an expert Snowflake data assistant. "
        ...
        if sql_blocks:
            ...
            interp = next(
                (
                    b["text"]
                    for b in resp_json["message"]["content"]
            ...
            df_summary = _local_short_summary(df) or _llm_question_and_answer(
    ...
    if sqls:
        ...
        interp = next(
            (
                b["text"]
                for b in rag_json["message"]["content"]
                if b["type"] == "text" and b["text"].strip()
            ),
            "",
        )
...
if __name__ == "__main__":
    print("🚀 Snowflake Intelligence Agent up")
